  ...
  if (tableInstance) {
    if (action === "query" && table.startsWith("$query")) {
      ...
      if (Array.isArray(json)) {
        ...
        // Validate UUID parameters - if query contains uuid() and param is invalid, return empty
        const queryStr = Array.isArray(q) ? q.join("") : String(q);
        if (queryStr.includes("uuid(")) {
          const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
          for (const param of json) {
            if (typeof param === "string" && !uuidRegex.test(param)) {
              // Invalid UUID parameter, return empty array instead of error
              return [];
            }
          }
        }
