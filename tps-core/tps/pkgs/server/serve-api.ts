...
export const serveAPI = async (url: URL, req: Request) => {
  ...
  if (found) {
    ...
    const current = {
      ...
      ...found,
      params,
      _url: url, // Add the normalized URL to context (after ...found to not be overwritten)
    };

    // Bind 'this' to current so apiContext(this) works correctly
    const finalResponse = await current.fn.call(current, ...args);
