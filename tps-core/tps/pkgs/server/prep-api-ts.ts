...
export const prepareAPITypes = async () => {
  ...
  for (const [k, v] of Object.entries(g.api)) {
    ...
    // Check file existence without quotes
    const checkPath = `app/srv/api/${v.path}`;
    const fileExists = await existsAsync(dir(checkPath));

    let p = {
      path: fileExists ? `"app/srv/api/${v.path}"` : `"pkgs/api/${v.path}"`,
      handler: fileExists
        ? `"./api/${v.path.substring(0, v.path.length - 3)}"`
        : `"../../pkgs/api/${v.path.substring(0, v.path.length - 3)}"`,
    };
